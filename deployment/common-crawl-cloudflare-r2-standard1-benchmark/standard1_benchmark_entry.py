#!/usr/bin/env python3
"""Entrypoint for the one-WAT, standard-1 Cloudflare Container benchmark."""
...
def main() -> int:
    ...
    sampler.start()
    benchmark_id = required("GROWTHSENT_BENCHMARK_ID")
    ...
    try:
        prefix = r2.normalize_key(benchmark.BENCHMARK_ROOT, benchmark_id)
        store = r2.R2Store.from_environment(allowed_prefixes=[prefix])
        result = benchmark.run_one_wat(
            ...
            json.dumps(
                {
                    ...
                    "container_wall_seconds": round(time.monotonic() - started, 3),
                    "completion_key": f"{prefix}/BENCHMARK-COMPLETED.json",
                },
                sort_keys=True,
            ),
            flush=True,
