#!/usr/bin/env bash
# Provision one guarded 1,000-WAT regional run after the local compilation gate has passed.
set -euo pipefail

if [[ $# -ne 1 || "$1" != "--approved-thousand-wat-run" ]]; then
  echo "Usage: $0 --approved-thousand-wat-run" >&2
  exit 2
fi
...
[[ -f "$SOURCE_MANIFEST" ]] || { echo "Missing locked 1,000-WAT source shard manifest: $SOURCE_MANIFEST" >&2; exit 1; }
NONCE="$(od -An -N4 -tx1 /dev/urandom | tr -d ' \n')"
TIMESTAMP="$(date -u +%Y%m%dT%H%M%SZ | tr '[:upper:]' '[:lower:]')"
RUN_ID="cc-main-2026-30-${TIMESTAMP}-standard1-regional-${NONCE}"
TEMP_ROOT="$(mktemp -d -t "growthsent-cloudflare-standard1-thousand-${RUN_ID}-XXXXXX")"
RUN_DIRECTORY="$TEMP_ROOT/bundle"
CONTROLLER_DIRECTORY="$TEMP_ROOT/controller"

echo "GrowthSent regional standard-1 1,000-WAT run (Ubuntu/WSL native)"
echo "Run ID: $RUN_ID"
echo "Scope: 1,000 locked WATs, four region-constrained Workers, four active standard-1 Containers per region, and 15-second lane spacing."
...
python3 "$SCRIPT_DIR/build_bundles.py" \
  ...
  --task-count 1000 \
  --execution-profile regional-thousand-wat \
...
set +e
printf '%s' "$PARENT_TOKEN" | GROWTHSENT_BOTO3_SITE_PACKAGES="$BOTO3_SITE_PACKAGES" node "$CONTROLLER_DIRECTORY/provision-and-start-wsl.mjs" --approved-thousand-wat-run "$RUN_DIRECTORY/RUN-PLAN.json"
RESULT=$?
unset PARENT_TOKEN
set -e
if [[ $RESULT -ne 0 ]]; then
  echo "1,000-WAT provisioner failed. Do not rerun it; an earlier regional lane may already be running." >&2
  exit "$RESULT"
fi
echo "Secret-free 1,000-WAT run context: $RUN_DIRECTORY/REGIONAL-RAMP-CONTEXT.json"
