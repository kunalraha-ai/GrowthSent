#!/usr/bin/env bash
# Compile the isolated 10,000-WAT regional plan locally. This has no remote side effects.
set -euo pipefail
...
echo "GrowthSent regional standard-1 10,000-WAT compilation gate (Ubuntu/WSL native)"
echo "Run ID: $RUN_ID"
echo "Scope: 10,000 locked WATs, four regional Workers, four active Containers per region."
...
# Host WSL Python intentionally does not need the Container-only pyarrow
# dependency. Run the focused contract suite in the reviewed image that the
# Wrangler dry-runs compile below.
TEST_IMAGE="growthsent-standard1-ten-thousand-gate-${NONCE}"
...
assert plan["execution_profile"] == "regional-ten-thousand-wat"
...
assert all(item["regional_task_count"] == 2500 for item in plan["regions"])
assert all(item["max_concurrent"] == 4 and item["max_instances"] == 6 for item in plan["regions"])
print("regional standard-1 10,000-WAT local plan gate passed")
PY

echo "SUCCESS: the 10,000-WAT regional plan compiled locally and passed four Wrangler dry-runs."
