#!/usr/bin/env bash
# Compile the isolated 1,000-WAT regional plan locally.  This has no remote side effects.
set -euo pipefail
...
echo "GrowthSent regional standard-1 1,000-WAT compilation gate (Ubuntu/WSL native)"
echo "Run ID: $RUN_ID"
echo "Scope: 1,000 locked WATs, four regional Workers, four active Containers per region."
...
TEST_IMAGE="growthsent-standard1-thousand-gate-${NONCE}"
...
assert plan["execution_profile"] == "regional-thousand-wat"
...
print("regional standard-1 1,000-WAT local plan gate passed")
PY

echo "SUCCESS: the 1,000-WAT regional plan compiled locally and passed four Wrangler dry-runs."
