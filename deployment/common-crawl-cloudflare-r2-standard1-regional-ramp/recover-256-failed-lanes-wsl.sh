#!/usr/bin/env bash
# Recover only terminal failed lanes while healthy source lanes keep running.
set -euo pipefail

if [[ $# -ne 1 || "$1" != "--approved-256-failed-lane-recovery" ]]; then
  echo "Usage: $0 --approved-256-failed-lane-recovery" >&2
  exit 2
fi
...
REQUESTED_RECOVERY_LANES="${GROWTHSENT_FAILED_LANE_RECOVERY_LANES:-}"
WSL_NODE_BIN="${GROWTHSENT_WSL_NODE_BIN:-$HOME/.local/share/growthsent-tools/node-v22.23.2-linux-x64/bin}"
[[ -n "$SOURCE_CONTEXT" ]] || { echo "Set GROWTHSENT_HIGH_CAPACITY_TEN_THOUSAND_RECOVERY_SOURCE_CONTEXT to HIGH-CAPACITY-TEN-THOUSAND-RAMP-CONTEXT.json." >&2; exit 2; }
...
[[ -f "$SOURCE_CONTEXT" ]] || { echo "Missing 256-slot source context: $SOURCE_CONTEXT" >&2; exit 1; }
if [[ -n "$REQUESTED_RECOVERY_LANES" ]] && ! [[ "$REQUESTED_RECOVERY_LANES" =~ ^(APAC-A|APAC-B|ENAM-A|ENAM-B|WNAM-A|WNAM-B|WEUR-A|WEUR-B)(,(APAC-A|APAC-B|ENAM-A|ENAM-B|WNAM-A|WNAM-B|WEUR-A|WEUR-B))*$ ]]; then
  echo "GROWTHSENT_FAILED_LANE_RECOVERY_LANES must be a comma-separated subset of the eight reviewed lane names." >&2
  exit 2
fi

NONCE="$(od -An -N4 -tx1 /dev/urandom | tr -d ' \n')"
TIMESTAMP="$(date -u +%Y%m%dT%H%M%SZ | tr '[:upper:]' '[:lower:]')"
RUN_ID="cc-main-2026-30-${TIMESTAMP}-s1-256lane-${NONCE}"
TEMP_ROOT="$(mktemp -d -t "growthsent-cloudflare-standard1-256-failed-lanes-${RUN_ID}-XXXXXX")"
RUN_DIRECTORY="$TEMP_ROOT/bundle"
CONTROLLER_DIRECTORY="$TEMP_ROOT/controller"
RECOVERY_CONTRACT="$TEMP_ROOT/FAILED-LANE-RECOVERY-CONTRACT.json"

echo "GrowthSent selective 256-slot 10,000-WAT failed-lane recovery (Ubuntu/WSL native)"
echo "Recovery run ID: $RUN_ID"
echo "Scope: only source Workers already terminal with recoverable failures are eligible. Healthy source lanes keep running and are excluded from the R2 credential, recovery task set, and new Workers."
[[ -z "$REQUESTED_RECOVERY_LANES" ]] || echo "Requested terminal source lanes: $REQUESTED_RECOVERY_LANES"
echo "Safety: source R2 output is read-only; recovery uses a hashed completion-marker inventory from only the terminal lane partitions; no object is overwritten."
echo "Each fresh recovery lane has at most 32 standard-1 slots and starts with the reviewed 30-second spacing."
...
cp "$SCRIPT_DIR/prepare-128-partial-recovery-wsl.mjs" "$CONTROLLER_DIRECTORY/prepare-failed-lane-recovery-wsl.mjs"
...
RECOVERY_SELECTOR=(--failed-lanes-only)
[[ -z "$REQUESTED_RECOVERY_LANES" ]] || RECOVERY_SELECTOR=("--failed-lanes-only=$REQUESTED_RECOVERY_LANES")
set +e
printf '%s' "$PARENT_TOKEN" | node "$CONTROLLER_DIRECTORY/prepare-failed-lane-recovery-wsl.mjs" "$SOURCE_CONTEXT" "$RECOVERY_CONTRACT" "${RECOVERY_SELECTOR[@]}"
RESULT=$?
set -e
if [[ $RESULT -ne 0 ]]; then
  unset PARENT_TOKEN
  echo "Read-only failed-lane recovery inventory did not pass. No Worker, Container, or R2 object was changed." >&2
  exit "$RESULT"
fi
...
if [[ "$RECOVERY_TASK_COUNT" == "0" ]]; then
  unset PARENT_TOKEN
  echo "SUCCESS: every eligible terminal-lane WAT already has a valid completion marker. No Worker or Container was deployed."
...
set -e
if [[ $RESULT -ne 0 ]]; then
  echo "Selective failed-lane recovery provisioner failed. Do not rerun it; a fresh recovery lane may already be running." >&2
  exit "$RESULT"
fi
echo "Secret-free recovery context: $RUN_DIRECTORY/HIGH-CAPACITY-TEN-THOUSAND-FAILED-LANE-RECOVERY-CONTEXT.json"
