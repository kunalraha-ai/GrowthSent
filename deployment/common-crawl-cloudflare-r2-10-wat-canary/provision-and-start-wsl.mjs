#!/usr/bin/env node
/**
 * Ubuntu/WSL-native provisioner for the bounded ten-WAT Container canary.
 * It accepts the raw Cloudflare parent API token only on stdin, mints a
 * prefix-scoped child through Cloudflare's API, and never prints credentials.
 */
...
const REFERENCE_BASELINE_PREFIX = "production/common-crawl/audit/public-source-baseline/v2/cc-main-2026-30-10-wat/";
const REFERENCE_BASELINE_KEY = `${REFERENCE_BASELINE_PREFIX}PUBLIC-SOURCE-BASELINE-MANIFEST.json`;
...
function safeCloudflareError(body) {
  ...
  return errors
    .map((item) => ({ code: item?.code ?? null, message: typeof item?.message === "string" ? item.message.slice(0, 240) : null }))
    .slice(0, 3);
}
...
function referenceBaselineConfig() {
  const key = process.env.GROWTHSENT_REFERENCE_BASELINE_KEY ?? REFERENCE_BASELINE_KEY;
  if (typeof key !== "string" || !/^production\/common-crawl\/audit\/public-source-baseline\/v2\/[a-z0-9][a-z0-9-]{0,63}\/PUBLIC-SOURCE-BASELINE-MANIFEST\.json$/.test(key)) {
    fail("The reference baseline key must be one immutable public-source-baseline v2 manifest.");
  }
  return { key, prefix: key.slice(0, key.lastIndexOf("/") + 1) };
}
...
async function mintChild(parentApiToken, parent, prefix, permission = "object-read-write") {
  ...
  if (!response.ok || !body?.success || typeof credentials?.accessKeyId !== "string" || !/^[0-9a-f]{64}$/.test(credentials?.secretAccessKey ?? "") || typeof credentials?.sessionToken !== "string") {
    fail(`Cloudflare Temporary Credentials API mint failed (${response.status}; ${JSON.stringify(safeCloudflareError(body))}).`);
  }
  return credentials;
}

async function verifyReferenceBaseline(credentials, expectedSha256, referenceBaselineKey) {
  ...
  const encodedKey = referenceBaselineKey.split("/").map(encodeURIComponent).join("/");
  const response = await client.fetch(`${endpoint}/${encodeURIComponent(BUCKET)}/${encodedKey}`, { method: "GET" });
  const body = Buffer.from(await response.arrayBuffer());
  if (!response.ok) fail(`Public-source baseline GetObject failed (HTTP ${response.status}).`);
  const actualSha256 = createHash("sha256").update(body).digest("hex");
  let document = null;
  try { document = JSON.parse(body.toString("utf8")); } catch { /* validated below */ }
  ...
  return { key: referenceBaselineKey, bytes: body.length, sha256: actualSha256 };
}
...
async function r2Preflight(credentials, prefix) {
  ...
  const completionKey = `${prefix}CANARY-COMPLETED.json`;
  const encodedKey = completionKey.split("/").map(encodeURIComponent).join("/");
  const getResponse = await client.fetch(`${endpoint}/${encodeURIComponent(BUCKET)}/${encodedKey}`, { method: "GET", headers: { Range: "bytes=0-0" } });
  const getBody = (await getResponse.text()).slice(0, 4096);
  const getResult = { http_status: getResponse.status, r2_error_code: xmlTag(getBody, "Code"), object_exists: getResponse.ok };
  if (getResult.http_status !== 404 || getResult.r2_error_code !== "NoSuchKey" || getResult.object_exists) {
    fail(`Child GetObject preflight failed (${getResult.http_status}; ${getResult.r2_error_code ?? "no R2 error code"}).`);
  }
  ...
  return { get: getResult, list: { http_status: listResponse.status, key_count: keyCount, truncated } };
}
...
async function boto3Preflight(bundleDirectory, credentials, prefix) {
  const request = JSON.stringify({
    ...
    bucket: BUCKET,
    key: `${prefix}CANARY-COMPLETED.json`,
    prefix,
    access_key_id: credentials.accessKeyId,
    secret_access_key: credentials.secretAccessKey,
    session_token: credentials.sessionToken,
  });
  const python = process.env.GROWTHSENT_BOTO3_PYTHON || "python3";
  const packagePath = process.env.GROWTHSENT_BOTO3_SITE_PACKAGES;
  const pythonPath = packagePath ? `${packagePath}${process.platform === "win32" ? ";" : ":"}${process.env.PYTHONPATH ?? ""}` : process.env.PYTHONPATH;
  const result = await run(python, [resolve(bundleDirectory, "r2-boto3-preflight.py")], {
    cwd: bundleDirectory,
    input: request,
...
async function workerStatus(workerUrl) {
  const response = await fetch(`${workerUrl}/_growthsent_canary/status`, { method: "GET" });
  if (!response.ok) return null;
  try { return await response.json(); } catch { return null; }
}

function workerUrlFromDeploy(output) {
  const match = /https:\/\/[a-z0-9][a-z0-9.-]*\.workers\.dev/i.exec(output);
  return match?.[0] ?? null;
}

async function main() {
  const [mode, bundleDirectoryArg] = process.argv.slice(2);
  if (!new Set(["--preflight-only", "--approved-ten-wat-canary"]).has(mode) || !bundleDirectoryArg) {
    fail("Usage: provision-and-start-wsl.mjs --preflight-only|--approved-ten-wat-canary <bundle-directory>");
  }
  ...
  const canaryId = config?.vars?.GROWTHSENT_CANARY_ID;
  ...
  if (typeof canaryId !== "string" || typeof workerName !== "string" || !/^[0-9a-f]{64}$/.test(releaseSha256 ?? "") || !/^[0-9a-f]{64}$/.test(referenceManifestSha256 ?? "") || hardTimeoutSeconds !== 6600) {
    fail("The local WSL bundle is not the reviewed bounded ten-WAT configuration.");
  }
  ...
  try {
    const prefix = `production/common-crawl/cloudflare-r2-canaries/v1/${canaryId}/`;
    const referenceBaseline = referenceBaselineConfig();
    const parent = await verifyParent(parentApiToken);
    const baselineReadCredentials = await mintChild(parentApiToken, parent, referenceBaseline.prefix, "object-read-only");
    const baseline = await verifyReferenceBaseline(baselineReadCredentials, referenceManifestSha256, referenceBaseline.key);
    ...
    const aws4fetch = await r2Preflight(childCredentials, prefix);
    json({ stage: "child_aws4fetch", ...aws4fetch });
    const boto3 = await boto3Preflight(bundleDirectory, childCredentials, prefix);
    json({ stage: "child_boto3", result: boto3 });
    if (mode === "--preflight-only") {
      json({ status: "preflight_ok", canary_id: canaryId, worker_deployed: false, container_started: false });
      return;
    }
    ...
    await writeFile(resolve(bundleDirectory, "CANARY-CONTEXT.json"), `${JSON.stringify({
      canary_id: canaryId,
    ...
    await runRequired("npx", [...WRANGLER, "secret", "bulk", "--name", workerName], { cwd: bundleDirectory, input: secrets });
    let status = null;
    ...
    if (!status || status.canary_id !== canaryId || status.control_secret_configured !== true || status.state !== "stopped" || status.terminal !== null) {
      fail("Fresh Worker/Container state was not safe to start; no Container start request was sent.");
    }
    const start = await fetch(`${workerUrl}/_growthsent_canary/start`, {
      method: "POST",
    ...
    if (start.status !== 202 || startResult?.accepted !== true || startResult?.canary_id !== canaryId) {
      fail(`Container start was not accepted (HTTP ${start.status}; ${safeStartError(startResult)}); no retry was attempted.`);
    }
    json({ stage: "start", http_status: 202, accepted: true, canary_id: canaryId });
    json({ status: "live_start_accepted", canary_id: canaryId, context: resolve(bundleDirectory, "CANARY-CONTEXT.json") });
  } finally {
    parentApiToken = "";
...
main().catch((error) => {
  const message = error instanceof Error ? error.message.replace(/[A-Za-z0-9_-]{40,}/g, "[redacted]").slice(0, 512) : "unknown WSL provisioner error";
