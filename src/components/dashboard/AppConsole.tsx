...
import LinkIntelligenceView from "./LinkIntelligenceView"
import AiReadinessView from "./AiReadinessView"
import {
  toAuditJobUiStatus,
  toAuditProgress,
  type AuditJobUiStatus,
  type AuditProgress,
} from "./audit-status"
...
const EMPTY_AUDIT_PROGRESS: AuditProgress = {
  status: null,
  progressPercent: 0,
  pagesCrawled: 0,
}
...
function AddWebsiteModal({
  ...
  onClose: () => void
  onAdded: (site: WebsiteItem, jobId?: string) => void
}) {
      ...
      >
        ...
        >
          ...
          <button
            onClick={onClose}
            className="close-btn"
            style={{ fontSize: "20px", color: "#171817", cursor: "pointer" }}
          >
            ✕
          </button>
        </div>
...
export function AppConsole({
  ...
  const [auditProgress, setAuditProgress] =
    useState<AuditProgress>(EMPTY_AUDIT_PROGRESS)
    ...
    async function loadWebsiteData() {
      try {
        const [scanResponse, gscResponse, gaResponse] = await Promise.all([
          ...
          fetch(
            `/api/v1/integrations?websiteId=${activeWebsiteId}&provider=google_search_console`,
          ),

          fetch(
            `/api/v1/integrations?websiteId=${activeWebsiteId}&provider=google_analytics`,
          ),
        ])
        ...
        if (gscResponse.ok)
          setIsGscConnected(Boolean((await gscResponse.json()).isConnected))

        if (gaResponse.ok)
          setIsGaConnected(Boolean((await gaResponse.json()).isConnected))
      } catch (error) {
  ...
  const pollAudit = (
    jobId: string,
    initialStatus: AuditJobUiStatus = "queued",
    retryUrl?: string,
  ) => {
    stopAuditPolling()
    setIsScanning(true)
    ...
    const pollOnce = async () => {
      ...
      try {
        ...
        if (!response.ok || !data) {
          ...
          setAuditFailure({
            message:
              "We could not check this audit’s progress. Please try again.",
            retryUrl,
          })
          return
        }
        ...
        setAuditFailure({
          message:
            "We lost the connection while checking this audit. Please try again.",
          retryUrl,
        })
      } finally {
        auditPollInFlightRef.current = false
    ...
    try {
      ...
      if (!website?._id) {
        ...
        if (!websiteResponse.ok || websiteData.error || !websiteData._id) {
          throw new Error(
            websiteData.error?.message || "Unable to add this website.",
          )
        }
        ...
        setWebsites((current) =>
          current.some((site) => site._id === websiteData._id)
            ? current
            : [...current, websiteData],
        )
      }
      ...
      const res = await fetch("/api/v1/audit", {
        ...
        body: JSON.stringify({
          url: targetUrl.includes("://") ? targetUrl : `https://${cleanHost}`,
      ...
      if (!res.ok || !data.jobId)
        throw new Error(data.error?.message || "Unable to start the audit.")

      pollAudit(
        data.jobId,
        toAuditJobUiStatus(data.status) || "queued",
        targetUrl,
      )
    } catch (error) {
      setIsScanning(false)
      ...
      setAuditFailure({
        message:
          error instanceof Error ? error.message : "Unable to start the audit.",
        retryUrl: targetUrl,
      })
    }
  }
  ...
  const retryAudit = () => {
    const retryUrl =
      auditFailure?.retryUrl || (activeSite ? `https://${activeSite}` : "")
  ...
  const createAuditShareLink = async (): Promise<string> => {
    ...
    if (typeof scanId !== "string" || !/^[a-f0-9]{24}$/i.test(scanId)) {
      throw new Error(
        "This completed audit is not ready to share yet. Refresh and try again.",
      )
    }
    const response = await fetch(`/api/v1/scans/${scanId}/share`, {
      method: "POST",
    })
    const data = await response.json().catch(() => null)
    if (
      ...
      !/^[A-Za-z0-9_-]{32,128}$/.test(data.token)
    ) {
      throw new Error(
        data?.error?.message ||
      ...
      <div className="full-app-layout">
        ...
        <Sidebar
          ...
          onDeleteWebsite={(site) =>
            handleWebsiteDeleted(site._id || site.hostname)
          }
        ...
        <div className="console-main">
            ...
            >
              <div>
                ...
                {auditFailure.code && (
                  ...
                  >
                    Reason: {auditFailure.code.replace(/_/g, " ").toLowerCase()}
                  </div>
                )}
              </div>
              <button
                ...
                type="button"
                onClick={retryAudit}
                disabled={isScanning || (!auditFailure.retryUrl && !activeSite)}
              >
                Try again
              </button>
            </div>
          ...
          {activeTab === "backlinks" && (
            <LinkIntelligenceView initialDomain={activeSite} />
          )}
      ...
      {showAddSite && (
        <AddWebsiteModal
          ...
          onAdded={(newSite, jobId) => {
            setWebsites((current) =>
              current.some((site) => site._id === newSite._id)
                ? current
                : [...current, newSite],
            )
