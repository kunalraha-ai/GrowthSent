...
import {
  handleDurableCrawlCronRequest,
  hasValidCronAuthorization,
} from "../jobs/cron-worker.js"
...
import {
  BacklinkProtectionError,
  enforceBacklinkRequestQuota,
} from "../backlinks/shared-protection.js"

import {
  LinkIntelligenceService,
  normalizeLinkIntelligenceDomain,
} from "../backlinks/link-intelligence.service.js"
...
export const AuthSignupSchema = z.object({
  ...
  turnstileToken: z
    .string()
    .max(2048, "Invalid human verification token")
    .optional(),
})
...
export const AuditInputSchema = z
  ...
  .object({
    ...
    websiteId: z
      .string()
      .regex(/^[a-f0-9]{24}$/i, "Invalid website identifier")
      .optional(),
  })
...
export const Ga4PropertySelectionSchema = z
  ...
  .object({
    ...
    propertyId: z
      ...
      .max(128, "propertyId is too long"),

    displayName: z
      .string()
      .trim()
      .max(256, "displayName is too long")
      .optional(),
  })

  .strict()

const GoogleProviderSchema = z.enum([
  "google_search_console",
  "google_analytics",
])

export function normalizeSearchIntelligenceDays(
...
function extractResourceAccessToken(
  ...
  if (headerToken && /^[A-Za-z0-9_-]{32,128}$/.test(headerToken))
    return headerToken
...
function safeErrorMetadata(
  error: unknown,
): { errorName: string; errorCode?: string | number } {
...
function googleSocialLoginFailureCategory(error: unknown): string {
  ...
  if (
    message ===
    "The Google sign-in request is invalid or has expired. Please try again."
  ) {
...
export async function handleApiRequest(req: ApiRequest): Promise<ApiResponse> {
  ...
  const isWebsiteCollectionRequest =
    path === "/api/v1/websites" && (method === "GET" || method === "POST")
  ...
  try {
    ...
    try {
      user = sessionToken
        ? await withMongoOperationPhase("session_validation", () =>
            validateSession(sessionToken),
          )
        : null
      if (isWebsiteCollectionRequest) {
        logWebsiteRouteTiming(
          "auth",
          authStartedAt,
          authStartedMs,
          "completed",
    ...
    if (method === "GET" && path === "/api/v1/backlinks") {
      ...
      if (!domain) {
        return {
          statusCode: 400,
          body: {
            error: { code: "INVALID_DOMAIN", message: "A domain is required." },
          },
        }
      }

      const page =
        req.query.page && /^\d+$/.test(req.query.page)
          ? Number.parseInt(req.query.page, 10)
          : undefined

      const pageSize =
        req.query.pageSize && /^\d+$/.test(req.query.pageSize)
    ...
    // LINK INTELLIGENCE (Common Crawl full-corpus aggregates)
    ...
    const linkIntelligenceDomainMatch = path.match(
      /^\/api\/v1\/link-intelligence\/([^/]+)\/summary$/,
    )

    if (method === "GET" && linkIntelligenceDomainMatch) {
      if (!user)
        return {
          statusCode: 401,
          body: {
      ...
      const domain = normalizeLinkIntelligenceDomain(
        decodeURIComponent(linkIntelligenceDomainMatch[1]),
      )
      ...
      return {
        statusCode: 200,
        body: { success: true, data: await service.getSummary(domain) },
      }
    }

    const linkIntelligenceRatingMatch = path.match(
      /^\/api\/v1\/link-intelligence\/([^/]+)\/rating$/,
    )

    if (method === "GET" && linkIntelligenceRatingMatch) {
      if (!user)
        return {
          statusCode: 401,
          body: {
      ...
      const domain = normalizeLinkIntelligenceDomain(
        decodeURIComponent(linkIntelligenceRatingMatch[1]),
      )
      ...
      return {
        statusCode: 200,
        body: { success: true, data: await service.getRating(domain) },
      }
    }

    const linkIntelligenceReferringDomainsMatch = path.match(
      /^\/api\/v1\/link-intelligence\/([^/]+)\/referring-domains$/,
    )

    if (method === "GET" && linkIntelligenceReferringDomainsMatch) {
      if (!user)
        return {
          statusCode: 401,
          body: {
      ...
      const domain = normalizeLinkIntelligenceDomain(
        decodeURIComponent(linkIntelligenceReferringDomainsMatch[1]),
      )

      const page = parseBoundedInt(req.query.page, 1, 10000, 1)

      const limit = parseBoundedInt(req.query.limit, 1, 100, 20)
      ...
      return {
        ...
        body: {
          success: true,
          data: await service.getReferringDomains(domain, { page, limit }),
        },
      }
    }

    const linkIntelligenceTopPagesMatch = path.match(
      /^\/api\/v1\/link-intelligence\/([^/]+)\/top-pages$/,
    )

    if (method === "GET" && linkIntelligenceTopPagesMatch) {
      if (!user)
        return {
          statusCode: 401,
          body: {
      ...
      const domain = normalizeLinkIntelligenceDomain(
        decodeURIComponent(linkIntelligenceTopPagesMatch[1]),
      )
      ...
      return {
        ...
        body: {
          success: true,
          data: await service.getTopPages(domain, { page, limit }),
        },
      }
    }

    const linkIntelligenceAnchorsMatch = path.match(
      /^\/api\/v1\/link-intelligence\/([^/]+)\/anchors$/,
    )

    if (method === "GET" && linkIntelligenceAnchorsMatch) {
      if (!user)
        return {
          statusCode: 401,
          body: {
      ...
      const domain = normalizeLinkIntelligenceDomain(
        decodeURIComponent(linkIntelligenceAnchorsMatch[1]),
      )

      const limit = parseBoundedInt(req.query.limit, 1, 200, 50)

      const service = new LinkIntelligenceService()

      return {
        statusCode: 200,
        body: { success: true, data: await service.getAnchors(domain, { limit }) },
      }
    }

    const linkIntelligenceBrokenMatch = path.match(
      /^\/api\/v1\/link-intelligence\/([^/]+)\/broken-backlinks$/,
    )

    if (method === "GET" && linkIntelligenceBrokenMatch) {
      if (!user)
        return {
          statusCode: 401,
          body: {
      ...
      const domain = normalizeLinkIntelligenceDomain(
        decodeURIComponent(linkIntelligenceBrokenMatch[1]),
      )
      ...
      const status = ["candidate", "verified_broken", "verified_live"].includes(
        req.query.status || "",
      )
        ? req.query.status as "candidate" | "verified_broken" | "verified_live"
        : undefined
      ...
      return {
        ...
        body: {
          success: true,
          data: await service.getBrokenBacklinks(domain, { page, limit, status }),
        },
      }
    }

    const linkIntelligenceGapMatch = path.match(
      /^\/api\/v1\/link-intelligence\/([^/]+)\/gap$/,
    )

    if (method === "GET" && linkIntelligenceGapMatch) {
      if (!user)
        return {
          statusCode: 401,
          body: {
      ...
      const domain = normalizeLinkIntelligenceDomain(
        decodeURIComponent(linkIntelligenceGapMatch[1]),
      )
      ...
      if (!req.query.competitor) {
        return {
          ...
          body: {
            error: {
              code: "INVALID_INPUT",
              message: "competitor query parameter is required.",
            },
          },
        }
      }

      const competitor = normalizeLinkIntelligenceDomain(req.query.competitor)
      ...
      return {
        ...
        body: {
          success: true,
          data: await service.getGap(domain, { competitor, page, limit }),
        },
      }
    }
    ...
    const integrationCallbackMatch =
    ...
    if (method === "GET" && integrationCallbackMatch) {
      ...
      if (!code || !state || !browserNonce) {
        return {
          ...
          headers: {
            Location: `${appUrl}${separator}integrationError=google`,
      ...
      try {
        const completed = await completeGoogleAuthorization(
          code,
          state,
          browserNonce,
        )
    ...
    if (method === "GET" && path === "/api/v1/integrations/google/start") {
      ...
      if (!providerResult.success || !websiteId) {
        return {
          ...
          body: {
            error: {
              code: "INVALID_INPUT",
              message: "A provider and websiteId are required.",
            },
          },
        }
      }
      ...
      if (!website)
        return {
          statusCode: 404,
          body: { error: { code: "NOT_FOUND", message: "Website not found." } },
        }
    ...
    if (method === "GET" && path === "/api/v1/integrations") {
      ...
      return {
        statusCode: 200,
        body: await getGoogleIntegrationStatus(
          user._id!.toString(),
          websiteId,
          providerResult.data,
        ),
    ...
    if (method === "DELETE" && path === "/api/v1/integrations") {
      ...
      const success = await disconnectGoogleIntegration(
        user._id!.toString(),
        websiteId,
        providerResult.data,
      )
    ...
    if (method === "GET" && path === "/api/v1/search-performance") {
      ...
      if (!websiteId)
        return {
          statusCode: 400,
          body: {
            error: { code: "INVALID_INPUT", message: "websiteId is required." },
          },
        }

      try {
        const queries = await fetchSearchConsoleKeywords(
          user._id!.toString(),
          websiteId,
        )
        ...
        return {
          ...
          body: {
            error: {
              ...
              message: isNotFound
                ? "No Google Search Console property is configured for this website."
    ...
    if (method === "GET" && path === "/api/v1/gsc-full-report") {
      ...
      try {
        const report = await fetchSearchConsoleFullReport(
          user._id!.toString(),
          websiteId,
          days,
        )
    ...
    if (method === "GET" && path === "/api/v1/ga4-properties") {
      ...
      const properties = await listGoogleAnalyticsProperties(
        user._id!.toString(),
        websiteId,
      )
          ...
          body: {
            error: {
              ...
              message:
                ...
                "Invalid Google Analytics property.",
            },
          },
        }
      }
    ...
    if (method === "GET" && path === "/api/v1/ga4-report") {
      ...
      try {
        const report = await fetchGoogleAnalyticsReport(
          user._id!.toString(),
          websiteId,
          days,
        )
          ...
          return {
            ...
            body: {
              error: {
                code: "NO_PROPERTY",
                message:
                  "No Google Analytics property is configured for this website.",
              },
            },
          }
        }
        ...
        if (/not connected/i.test(msg) || /no active.*integration/i.test(msg)) {
          return {
            ...
            body: {
              error: {
                code: "NOT_CONNECTED",
                message: "Google Analytics is not connected for this website.",
              },
            },
          }
        }
        ...
        return {
          ...
          body: {
            error: {
              code: "GA4_ERROR",
              message: "Unable to fetch Google Analytics data.",
            },
          },
        }
      }
    ...
    if (method === "GET" && path === "/api/v1/ga4-full-report") {
      ...
      try {
        const report = await fetchGa4FullReport(
          user._id!.toString(),
          websiteId,
          days,
        )
    ...
    if (method === "POST" && path === "/api/v1/audit") {
        ...
        return {
          ...
          body: {
            error: {
              code: "INVALID_INPUT",
              message:
                parsed.error.issues[0]?.message || "Invalid audit request.",
            },
          },
        }
      }
      ...
      if (websiteId) {
        ...
        const website = await withMongoOperationPhase(
      ...
      let job
      try {
        job = await AuditService.createCrawlJob(
          targetUrl,
          user?._id?.toString(),
          websiteId,
          req.ip,
      ...
      return {
        ...
        backgroundTask: job.reused
          ? undefined
          : () => dispatchImmediateAuditJob(job.jobId),
      }
    }
    ...
    if (method === "GET" && auditMatch) {
      ...
      const statusData = await AuditService.getCrawlJobStatus(jobId, {
        ...
        accessToken: extractResourceAccessToken(
          req,
          AUDIT_ACCESS_COOKIE,
          jobId,
    ...
    const shareCreateMatch = path.match(
      /^\/api\/v1\/scans\/([a-f0-9]{24})\/share$/,
    )
    if (method === "POST" && shareCreateMatch) {
      ...
      const token = await createScanShareToken(
        shareCreateMatch[1],
        user._id!.toString(),
      )
      if (!token)
        return {
          statusCode: 404,
          body: {
            error: { code: "NOT_FOUND", message: "Completed audit not found." },
          },
        }
      return {
        statusCode: 201,
    ...
    const sharedAuditMatch = path.match(
      /^\/api\/v1\/shared\/audits\/([A-Za-z0-9_-]{32,128})$/,
    )
    if (method === "GET" && sharedAuditMatch) {
      ...
      if (!report)
        return {
          statusCode: 404,
          body: {
            error: { code: "NOT_FOUND", message: "Shared audit not found." },
          },
        }
      return {
        statusCode: 200,
      ...
      return {
        ...
        body: {
          error: {
            ...
            message:
              "This scan endpoint is disabled for the external MVP. Use the bounded audit flow instead.",
          },
        },
      }
    }
    ...
    if (method === "GET" && path === "/api/v1/auth/google/start") {
      ...
      try {
        ...
        console.error(
          "[auth] Google social sign-in could not start.",
          safeErrorMetadata(error),
        )
        return {
          statusCode: 503,
    ...
    if (method === "GET" && path === "/api/v1/auth/google/callback") {
      ...
      if (!code || !state) {
        return {
          ...
          headers: {
            Location: `${appUrl}/?authError=Google+sign-in+failed`,
            "Set-Cookie": clearNonceCookie,
            "Cache-Control": "no-store",
          },
      ...
      try {
        const loggedInUser = await completeGoogleLogin(
          code,
          state,
          browserNonce,
        )
        ...
        return {
          statusCode: 302,
          headers: {
            Location: getDashboardRedirect(appUrl),
            "Set-Cookie": [
      ...
      try {
        ...
        return {
          ...
          body: {
            error: {
              code: "OAUTH_UNAVAILABLE",
              message: "GitHub sign-in is temporarily unavailable.",
            },
          },
        }
      }
    ...
    if (method === "GET" && path === "/api/v1/auth/github/callback") {
      ...
      if (!code || !state) {
        return {
          ...
          headers: {
            Location: `${appUrl}/?authError=GitHub+sign-in+failed`,
            "Set-Cookie": clearNonceCookie,
            "Cache-Control": "no-store",
          },
      ...
      try {
        const loggedInUser = await completeGithubLogin(
          code,
          state,
          browserNonce,
        )
    ...
    if (method === "POST" && path === "/api/v1/auth/signup") {
      ...
      const humanVerification = await validateTurnstileToken(
        parse.data.turnstileToken,
        req.ip,
      )
    ...
    if (method === "GET" && path === "/api/v1/websites") {
      ...
      try {
        ...
        logWebsiteRouteTiming(
          "owner_validation",
          ownerValidationStartedAt,
          ownerValidationStartedMs,
          "completed",
          {
            ownerIdValid: true,
          },
    ...
    const aiReadinessMatch = path.match(
      /^\/api\/v1\/websites\/([a-f0-9]{24})\/ai-readiness$/,
    )
    if (method === "POST" && aiReadinessMatch) {
      ...
      const readinessLimit = checkRateLimit(
        `ai-readiness:${user._id!.toString()}:${websiteId}`,
        {
          maxRequests: 3,
          windowMs: 60 * 1000,
        },
      ...
      if (!readinessLimit.allowed) {
        return {
          ...
          body: {
            error: {
              code: "AI_READINESS_RATE_LIMITED",
              message:
                "Please wait a minute before running another AI readiness audit for this website.",
            },
          },
        }
      }
        ...
        return {
          ...
          body: {
            error: {
              ...
              message:
                "This website cannot be fetched safely for an AI readiness audit. Check its public hostname and try again.",
            },
          },
        }
      }
    ...
    const latestWebScanMatch = path.match(
      /^\/api\/v1\/websites\/([a-f0-9]{24})\/latest-scan$/,
    )
    if (method === "GET" && latestWebScanMatch) {
      ...
      if (!scan?._id)
        return { statusCode: 200, body: { scan: null, pages: [], issues: [] } }

      const [pages, issues] = await Promise.all([
        getScanPages(scan._id.toString()),
        getScanIssues(scan._id.toString()),
      ])

      return {
        statusCode: 200,
        body: { scan: toPublicScan(scan), pages, issues },
      }
    }
    ...
    const webScansMatch = path.match(
      /^\/api\/v1\/websites\/([a-f0-9]{24})\/scans$/,
    )
      ...
      return {
        ...
        body: {
          error: {
            code: "LEGACY_SCAN_DISABLED",
            message:
              "This scan endpoint is disabled for the external MVP. Start a bounded audit from the website dashboard.",
          },
        },
      }
    }
    ...
    const analyticsCollectMatch = path.match(
      /^\/api\/v1\/websites\/([a-f0-9]{24})\/analytics\/collect$/,
    )
    ...
    const analyticsSummaryMatch = path.match(
      /^\/api\/v1\/websites\/([a-f0-9]{24})\/analytics$/,
    )
    ...
    logProductionApiHandlerError("API handler failed.", err, {
      route: path,
      method,
    })
    return {
