...
// Ensure Node process uses Google/Cloudflare DNS to resolve MongoDB SRV records on Windows/local networks
try {
...
/** Vite dev middleware serving GrowthSent API routes during local development */
function growthsentApiMiddleware(): Plugin {
  return {
    name: 'growthsent-api-middleware',
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        const url = req.url || ''
        ...
        try {
          const handlerModule = await server.ssrLoadModule('./api/v1/[...path].ts')
